...
export default function GlobalForm<T extends FieldValues>({
    ...
    const getRegisterOptions = (field: FormField<T>) => ({
        ...field.validation,
        ...(field.matchField && {
            validate: (value: unknown) => value === values[field.matchField] || `${field.label} does not match`
        })
    });
                    ...
                    <div className={`grid gap-6 ${isTwoColumns ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1"}`}>
                        {fields.map((field) => (
                            <div key={String(field.name)} className="space-y-2">
                                ...
                                {field.type === "select" ? (
                                    <select {...register(field.name, getRegisterOptions(field))} disabled={field.disabled} className={inputClass}>
                                    ...
                                    <textarea rows={5} {...register(field.name, getRegisterOptions(field))} placeholder={field.placeholder} disabled={field.disabled} className={textareaClass}/>
                                ) : (
                                    ...
                                    <div className="relative">
                                        ...
                                        {field.type === "password" && (
                                            ...
                                            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 transition hover:text-blue-600">
                        ...
                        <div className={`flex items-end justify-center ${isTwoColumns && fields.length % 2 === 0 ? "md:col-span-2" : ""}`}>
                            ...
                            <button type="submit" className="min-w-44 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-3 font-semibold text-white shadow-lg transition duration-300 hover:scale-105 hover:shadow-xl active:scale-95">
                                {submitLabel}
                            </button>
                        </div>
                    </div>
